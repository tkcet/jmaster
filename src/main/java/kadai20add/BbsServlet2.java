...
public class BbsServlet2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("write")) {
			...
			String str = sdate + "　" + name + "：" + request.getParameter("TEXT");
			@SuppressWarnings("unchecked")
			List<String> text = (List<String>) session.getAttribute("CONTENT");
			if (text == null) {
				text = new ArrayList<>();
				session.setAttribute("CONTENT", text);
			}
			text.add(str);
			
			RequestDispatcher rd = request.getRequestDispatcher("/ShowServlet"); // STEP2
			...
			out.println("<form action=\"http://localhost:8080/jmaster/kadai20add/bbs2.html\" method=\"POST\">");
			out.println("操作エラー<br>");
			out.println("<a href = \"http://localhost:8080/jmaster/kadai20add/bbs2.html\">戻る</a>");
			out.println("</form>");
